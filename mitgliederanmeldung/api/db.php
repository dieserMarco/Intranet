...
function is_dev_token(?string $token): bool {
    $allow = env_value('ALLOW_DEV_TOKEN', '0') === '1';
    $devToken = env_value('DEV_TOKEN');

    if (!$allow || $devToken === null || $token === null) {
        return false;
    }

    return hash_equals($devToken, trim($token));
}
